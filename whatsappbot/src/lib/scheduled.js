// src/lib/scheduled.js (17 languages)
...
function buildMidstayUpsell(guest, hotel, partners, lang, nightsLeft) {
  ...
  return t({
    ...
    de: `Hallo ${name}! 👋 Sie haben noch ${nightsLeft} Nächte!\n\n${sugg}\n\nInteresse? Ich buche sofort! 🌴`,
    fr: `Bonjour ${name}! 👋 Il vous reste ${nightsLeft} nuits!\n\n${sugg}\n\nIntéressé? Je réserve tout de suite! 🌴`,
    zh: `您好 ${name}！👋 您还有 ${nightsLeft} 晚！\n\n${sugg}\n\n感兴趣？我马上预订！🌴`,
    pl: `Cześć ${name}! 👋 Masz jeszcze ${nightsLeft} nocy!\n\n${sugg}\n\nCoś interesuje? Zarezerwuję od razu! 🌴`,
    sv: `Hej ${name}! 👋 Du har fortfarande ${nightsLeft} nätter!\n\n${sugg}\n\nIntresserad? Jag bokar direkt! 🌴`,
    fi: `Hei ${name}! 👋 Sinulla on vielä ${nightsLeft} yötä!\n\n${sugg}\n\nKiinnostaa? Varaan heti! 🌴`,
    uk: `Привіт, ${name}! 👋 У вас ще ${nightsLeft} ночей!\n\n${sugg}\n\nЗацікавило? Забронюю одразу! 🌴`,
    ar: `مرحباً ${name}! 👋 لا يزال لديك ${nightsLeft} ليالٍ!\n\n${sugg}\n\nمهتم؟ سأحجز فوراً! 🌴`,
    nl: `Hallo ${name}! 👋 U heeft nog ${nightsLeft} nachten!\n\n${sugg}\n\nGeïnteresseerd? Ik boek het meteen! 🌴`,
    el: `Γεια ${name}! 👋 Έχεις ακόμη ${nightsLeft} βράδια!\n\n${sugg}\n\nΕνδιαφέρεσαι; Κάνω κράτηση αμέσως! 🌴`,
    es: `¡Hola ${name}! 👋 ¡Aún te quedan ${nightsLeft} noches!\n\n${sugg}\n\n¿Te interesa? ¡Lo reservo ahora mismo! 🌴`,
    ca: `Hola ${name}! 👋 Encara tens ${nightsLeft} nits!\n\n${sugg}\n\nT'interessa? Ho reservo ara mateix! 🌴`,
    it: `Ciao ${name}! 👋 Hai ancora ${nightsLeft} notti!\n\n${sugg}\n\nTi interessa? Lo prenoto subito! 🌴`,
    pt: `Olá ${name}! 👋 Ainda tem ${nightsLeft} noites!\n\n${sugg}\n\nInteressa? Reservo agora mesmo! 🌴`,
  }, lang)
}
...
function buildFeedbackFollowup(rating, hotel, lang, tripadvisorUrl) {
  ...
  const high = {
    ...
    ru: `Большое спасибо! 🌟\n\nОтзыв на TripAdvisor очень важен для нас:\n${url}\n\nБудем рады видеть вас снова! 🌴`,
    ...
    fr: `Merci beaucoup! 🌟\n\nUn avis sur TripAdvisor nous aiderait beaucoup:\n${url}\n\nÀ très bientôt! 🌴`,
    zh: `非常感谢！🌟\n\n如果方便，请在TripAdvisor上留下评价：\n${url}\n\n期待再次欢迎您！🌴`,
    ...
    fi: `Paljon kiitoksia! 🌟\n\nArvostelu TripAdvisorissa auttaisi meitä paljon:\n${url}\n\nToivottavasti näemme sinut pian uudelleen! 🌴`,
    uk: `Велике дякую! 🌟\n\nВідгук на TripAdvisor буде дуже важливим:\n${url}\n\nЧекаємо знову! 🌴`,
    ar: `شكراً جزيلاً! 🌟\n\nمراجعة على TripAdvisor ستعني لنا الكثير:\n${url}\n\nنأمل رؤيتك مرة أخرى! 🌴`,
    nl: `Heel erg bedankt! 🌟\n\nEen review op TripAdvisor zou veel voor ons betekenen:\n${url}\n\nTot ziens! 🌴`,
    el: `Ευχαριστούμε πολύ! 🌟\n\nΜια αξιολόγηση στο TripAdvisor θα μας βοηθήσει πολύ:\n${url}\n\nΕλπίζουμε να σε δούμε ξανά! 🌴`,
    es: `¡Muchas gracias! 🌟\n\nUna reseña en TripAdvisor nos ayudaría muchísimo:\n${url}\n\n¡Esperamos verte pronto! 🌴`,
    ca: `Moltes gràcies! 🌟\n\nUna ressenya a TripAdvisor ens ajudaria molt:\n${url}\n\nEsperem veure't aviat! 🌴`,
    it: `Grazie mille! 🌟\n\nUna recensione su TripAdvisor significherebbe molto per noi:\n${url}\n\nSperiamo di rivederti presto! 🌴`,
    pt: `Muito obrigado! 🌟\n\nUma avaliação no TripAdvisor significaria muito para nós:\n${url}\n\nEsperamos vê-lo novamente! 🌴`,
  }
  const low = {
    ...
    ar: `شكراً على ملاحظاتك الصادقة. نحن آسفون.\n\nما الذي كان يمكننا فعله بشكل أفضل؟ ملاحظاتك ستذهب إلى مديرنا.`,
    nl: `Bedankt voor uw eerlijke feedback. Het spijt ons.\n\nWat hadden we beter kunnen doen? Uw feedback gaat naar onze manager.`,
    el: `Ευχαριστούμε για την ειλικρινή σου γνώμη. Λυπόμαστε.\n\nΤι θα μπορούσαμε να κάναμε καλύτερα; Η γνώμη σου πηγαίνει στον διευθυντή μας.`,
    es: `Gracias por tu opinión sincera. Lo sentimos mucho.\n\n¿Qué podríamos haber hecho mejor? Tu opinión llega directamente a nuestro director.`,
    ca: `Gràcies per la teva opinió sincera. Ho sentim molt.\n\nQu'hauríem pogut fer millor? La teva opinió arribarà directament al nostre director.`,
    it: `Grazie per il tuo feedback onesto. Ci dispiace molto.\n\nCosa avremmo potuto fare meglio? Il tuo feedback va direttamente al nostro direttore.`,
    pt: `Obrigado pelo seu feedback honesto. Lamentamos muito.\n\nO que poderíamos ter feito melhor? O seu feedback vai diretamente para o nosso diretor.`,
  }
...
export async function processScheduledMessages(hotelId) {
  ...
  for (const guest of (guests || [])) {
    ...
    async function shouldSend(type) {
      const { data } = await supabase.from('scheduled_messages').select('id,status').eq('guest_id', guest.id).eq('message_type', type).single()
      return !(data?.status === 'sent' || data?.status === 'skipped')
    }

    async function markSent(type) {
      await supabase.from('scheduled_messages').upsert({ hotel_id: hotelId, guest_id: guest.id, message_type: type, status: 'sent', sent_at: now.toISOString() }, { onConflict: 'guest_id,message_type' })
    }

    async function markSkipped(type) {
      await supabase.from('scheduled_messages').upsert({ hotel_id: hotelId, guest_id: guest.id, message_type: type, status: 'skipped', sent_at: now.toISOString() }, { onConflict: 'guest_id,message_type' })
    }
    ...
    async function send(type, message) {
      try {
        ...
        await markSent(type)
        const { data: conv } = await supabase.from('conversations').select('id,messages').eq('guest_id', guest.id).in('status',['active','escalated']).order('created_at',{ascending:false}).limit(1).single()
        if (conv) {
          const messages = [...(conv.messages||[]), { role:'assistant', content:message, ts:now.toISOString(), sent_by:'scheduled' }]
          await supabase.from('conversations').update({ messages, last_message_at: now.toISOString() }).eq('id', conv.id)
        }
        results.sent++
      } catch(e) {
        await supabase.from('scheduled_messages').upsert({ hotel_id: hotelId, guest_id: guest.id, message_type: type, status: 'failed', error: e.message }, { onConflict: 'guest_id,message_type' })
        results.failed++
        results.errors.push(`${guest.name}/${type}: ${e.message}`)
      }
    }

    const d7 = new Date(checkinDate); d7.setDate(d7.getDate()-7); d7.setHours(10,0,0,0)
    if (now >= d7 && now < checkinDate && await shouldSend('pre_checkin_7d'))
      await send('pre_checkin_7d', buildPreCheckin7d(guest, hotel, lang))

    const d1 = new Date(checkinDate); d1.setDate(d1.getDate()-1); d1.setHours(10,0,0,0)
    if (now >= d1 && now < checkinDate && await shouldSend('pre_checkin_24h'))
      await send('pre_checkin_24h', buildPreCheckin24h(guest, hotel, lang))

    const m1s = new Date(checkinDate); m1s.setDate(m1s.getDate()+1); m1s.setHours(9,0,0,0)
    const m1e = new Date(m1s); m1e.setHours(12,0,0,0)
    if (now >= m1s && now < m1e && now < checkoutDate && await shouldSend('day1_upsell'))
      await send('day1_upsell', buildDay1Upsell(guest, hotel, partners||[], lang))

    if (stayNights >= 6) {
      const mid = new Date(checkinDate); mid.setDate(mid.getDate()+Math.floor(stayNights/2)); mid.setHours(11,0,0,0)
      const midE = new Date(mid); midE.setHours(14,0,0,0)
      const nightsLeft = Math.round((checkoutDate - now) / (1000*60*60*24))
      if (now >= mid && now < midE && now < checkoutDate && nightsLeft >= 2 && await shouldSend('midstay_upsell'))
        await send('midstay_upsell', buildMidstayUpsell(guest, hotel, partners||[], lang, nightsLeft))
    } else {
      await markSkipped('midstay_upsell')
    }

    const preOut = new Date(checkoutDate); preOut.setDate(preOut.getDate()-1); preOut.setHours(17,0,0,0)
    const preOutE = new Date(preOut); preOutE.setHours(20,0,0,0)
    if (now >= preOut && now < preOutE && await shouldSend('pre_checkout'))
      await send('pre_checkout', buildPreCheckout(guest, hotel, lang))

    const postOut = new Date(checkoutDate); postOut.setHours(13,0,0,0)
    const postOutE = new Date(postOut); postOutE.setHours(18,0,0,0)
    if (now >= postOut && now < postOutE && now > checkoutDate && await shouldSend('post_checkout'))
...
export async function handleFeedbackReply(from, message, hotelId) {
  ...
  const { data: guest } = await supabase.from('guests').select('*').eq('phone', from).eq('hotel_id', hotelId).single()
  if (!guest) return false

  const { data: scheduled } = await supabase.from('scheduled_messages').select('*').eq('guest_id', guest.id).eq('message_type', 'post_checkout').eq('status', 'sent').single()
  if (!scheduled) return false

  const { data: existing } = await supabase.from('guest_feedback').select('id').eq('guest_id', guest.id).single()
  if (existing) return false

  await supabase.from('guest_feedback').insert({ hotel_id: hotelId, guest_id: guest.id, rating, language: guest.language || 'en' })
