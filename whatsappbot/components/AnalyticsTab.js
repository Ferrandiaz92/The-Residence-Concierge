...
export default function AnalyticsTab({ hotelId }) {
  ...
  // Heatmap data: [morning, afternoon, night, latenight] per day Mon-Sun
  const heatmap = [
    [20, 35, 45, 8],   // Mon
    [22, 38, 42, 9],   // Tue
    [25, 52, 58, 14],  // Wed
  ...
  function heatColor(val) {
    ...
    if (intensity < 0.2) return '#E8F0EC'
    if (intensity < 0.4) return '#C8DDD4'
    if (intensity < 0.6) return '#9FC5BA'
    if (intensity < 0.8) return '#3D7A6A'
  ...
  const kpis = [
    { label: 'Monthly commission', value: `€${(s.totalCommission || totalComm).toLocaleString()}`, sub: '+18% vs last month', subColor: 'var(--success)', accent: true },
    { label: 'Total bookings', value: s.totalBookings || 187, sub: '+24 this week', subColor: 'var(--success)' },
    { label: 'Avg response time', value: '18s', sub: 'vs 4hrs manual', subColor: 'var(--success)' },
    { label: 'Guest satisfaction', value: '4.8', sub: '+0.3 vs last month', subColor: 'var(--success)' },
    { label: 'Automation rate', value: '84%', sub: 'handled by bot', subColor: 'var(--success)' },
  ]
  ...
  return (
    <div style={{ height: '100%', overflow: 'auto', padding: '14px', background: 'var(--gray-50)', fontFamily: 'var(--font)' }}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '8px', marginBottom: '12px' }}>
        {kpis.map(k => (
          <div key={k.label} style={{
            background: k.accent ? 'rgba(201,168,76,0.06)' : 'white',
            border: `0.5px solid ${k.accent ? 'var(--gold)' : 'var(--border)'}`,
            ...
            <div style={{ fontSize: '9px', color: k.subColor, marginTop: '3px' }}>{k.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
        ...
        {card(<>
          ...
          {Object.entries(commByCategory).map(([type, amount]) => (
            <div key={type} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
              <div style={{ fontSize: '9px', color: 'var(--gray-500)', width: '72px', textAlign: 'right', flexShrink: 0 }}>
                {type.replace(/_/g,' ')}
              </div>
              <div style={{ flex: 1, height: '8px', background: 'var(--gray-100)', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ width: `${(amount/maxComm)*100}%`, height: '100%', background: 'var(--green-800)', borderRadius: '4px' }} />
              </div>
        ...
        {card(<>
          ...
          {internalServices.map(s => (
            <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
              ...
              <div style={{ flex: 1, height: '8px', background: 'var(--gray-100)', borderRadius: '4px', overflow: 'hidden' }}>
                <div style={{ width: `${(s.value/maxInternal)*100}%`, height: '100%', background: s.color, borderRadius: '4px' }} />
              </div>
        ...
        {card(<>
          ...
          {issueCategories.map(ic => {
            ...
            const resPct = (ic.resolved / total) * 100
            const openPct = (ic.open / total) * 100
            return (
              <div key={ic.label} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '7px' }}>
                ...
                <div style={{ flex: 1, height: '8px', background: 'var(--gray-100)', borderRadius: '4px', overflow: 'hidden', display: 'flex' }}>
                  <div style={{ width: `${resPct}%`, height: '100%', background: 'var(--green-800)' }} />
                  <div style={{ width: `${openPct}%`, height: '100%', background: '#D94040' }} />
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '10px', marginBottom: '10px' }}>
        ...
        {card(<>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
            ...
            <div style={{ flex: 1, height: '18px', background: 'var(--gray-100)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', display: 'flex' }}>
              <div style={{ width: '79%', height: '100%', background: 'var(--green-800)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: '9px', fontWeight: '600', color: 'white' }}>79% Yes</span>
              </div>
              <div style={{ width: '21%', height: '100%', background: '#D94040', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <span style={{ fontSize: '9px', fontWeight: '600', color: 'white' }}>21%</span>
              </div>
            </div>
          </div>
        ...
        {card(<>
          ...
          <div style={{ display: 'flex', gap: '3px', paddingLeft: '76px', marginBottom: '4px' }}>
            {days.map(d => (
              <div key={d} style={{ flex: 1, fontSize: '8px', color: 'var(--gray-400)', textAlign: 'center' }}>{d}</div>
            ))}
          </div>
          {/* Heatmap rows */}
          {slots.map((slot, si) => (
            <div key={slot} style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
              <div style={{ width: '70px', fontSize: '9px', color: 'var(--gray-500)', textAlign: 'right', flexShrink: 0, lineHeight: '1.3', whiteSpace: 'pre-line' }}>
                {slot}
              </div>
              <div style={{ flex: 1, display: 'flex', gap: '3px' }}>
                {days.map((d, di) => (
                  <div key={d} style={{
                    flex: 1, height: '22px',
                    background: heatColor(heatmap[di][si]),
                    borderRadius: '3px',
                  }} />
                ))}
              </div>
          ...
          {[
            ...
            <div key={p.rank} style={{ background: 'var(--gray-50)', borderRadius: 'var(--radius-md)', padding: '9px 10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '5px' }}>
                <div style={{ width: '20px', height: '20px', borderRadius: 'var(--radius-sm)', background: 'var(--green-100)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: '700', color: 'var(--green-800)', flexShrink: 0 }}>
                  {p.rank}
                </div>
