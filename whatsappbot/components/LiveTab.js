// components/LiveTab.js — updated
// Key changes:
// 1. Shows escalated conversations in left column (not hidden)
// 2. Maintenance/housekeeping roles see DepartmentQueue instead
// 3. Conversation persistence — reuses existing conversation per guest stay
...
const isDeptRole = (role) => ['maintenance','housekeeping','concierge','fnb'].includes(role)

export default function LiveTab({ hotelId, session, onSelectGuest }) {
  // If maintenance/housekeeping — show their ticket queue instead
...
function ReceptionistView({ hotelId, session, onSelectGuest }) {
  ...
  return (
    <div style={{ display:'grid',gridTemplateColumns:'190px 1fr 220px',height:'100%',overflow:'hidden' }}>
      ...
      {/* LEFT: Conversations — shows both active AND escalated */}
          ...
          {conversations.map(conv => {
            ...
            return (
              <div key={conv.id} onClick={() => handleConvClick(conv)}
                style={{ padding:'9px 10px',borderBottom:'0.5px solid var(--border)',borderLeft:isActive?'2px solid var(--gold)':isEscalated?'2px solid #D94040':'2px solid transparent',background:isActive?'rgba(201,168,76,0.05)':isEscalated?'rgba(217,64,64,0.03)':'white',cursor:'pointer' }}
                onMouseEnter={e=>{ if (!isActive) e.currentTarget.style.background='var(--gray-50)' }}
                onMouseLeave={e=>{ if (!isActive) e.currentTarget.style.background=isEscalated?'rgba(217,64,64,0.03)':'white' }}
              >
                ...
                <div style={{ fontSize:'10px',color:'var(--gray-400)',marginTop:'2px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:'155px' }}>
                  {isEscalated && <span style={{ color:'#D94040',fontSize:'9px',fontWeight:'700',marginRight:'4px' }}>REPLY NEEDED</span>}
                  {!isEscalated && last?.content}
                </div>
                <div style={{ marginTop:'3px',display:'flex',gap:'4px' }}>
                  ...
                  {isEscalated && <span style={{ fontSize:'8px',fontWeight:'700',padding:'1px 5px',borderRadius:'4px',background:'var(--red-light)',color:'var(--red)' }}>escalated</span>}
                </div>
              </div>
            )
          })}
        ...
        <div style={{ background:'white',borderBottom:'0.5px solid var(--border)',padding:'6px 12px',display:'flex',alignItems:'center',gap:'8px',flexShrink:0 }}>
          {['chat','portal'].map(mode => (
            <button key={mode} onClick={() => setCentreMode(mode)}
              style={{ padding:'4px 12px',borderRadius:'20px',fontSize:'11px',border:'0.5px solid',borderColor:centreMode===mode?'var(--green-800)':'var(--border-md)',background:centreMode===mode?'var(--green-800)':'white',color:centreMode===mode?'white':'var(--gray-500)',cursor:'pointer',fontFamily:'var(--font)' }}>
              {mode === 'chat' ? 'Chat thread' : 'Staff portal'}
            </button>
          ))}
          {selectedConv && centreMode === 'chat' && (
            <div style={{ fontSize:'11px',color:'var(--gray-400)',marginLeft:'4px' }}>
              ...
              {selectedConv.status === 'escalated' && <span style={{ color:'#D94040',fontWeight:'600',marginLeft:'6px' }}>· Needs reply</span>}
            </div>
          )}
        </div>
              ...
              <>
                <div className="scrollable" style={{ padding:'14px 16px',display:'flex',flexDirection:'column',gap:'6px' }}>
                  {(selectedConv.messages || []).length === 0 && (
                    <div style={{ textAlign:'center',color:'var(--gray-400)',fontSize:'12px',padding:'20px' }}>No messages yet</div>
                  )}
                ...
                {canReply(session?.role) ? (
                  <div style={{ padding:'10px 14px',background:'white',borderTop:'0.5px solid var(--border)',display:'flex',gap:'8px',alignItems:'flex-end',flexShrink:0 }}>
                    <textarea value={replyText} onChange={e=>setReplyText(e.target.value)}
                      ...
                      style={{ flex:1,height:'52px',padding:'8px 10px',border:'0.5px solid var(--border-md)',borderRadius:'var(--radius-md)',fontSize:'12px',fontFamily:'var(--font)',resize:'none',outline:'none',color:'var(--gray-800)' }}
                    />
                    <button onClick={handleReply} disabled={sending||!replyText.trim()}
                      style={{ padding:'8px 16px',background:replyText.trim()?'var(--green-800)':'var(--gray-200)',border:'none',borderRadius:'var(--radius-md)',fontSize:'12px',fontWeight:'500',color:replyText.trim()?'white':'var(--gray-400)',cursor:replyText.trim()?'pointer':'not-allowed',fontFamily:'var(--font)',flexShrink:0,height:'52px' }}>
                      {sending?'...':'Send'}
                    </button>
                  </div>
                ) : (
